...
public sealed class SelfUpdater
{
    ...
    /// <summary>
    /// Downloads the new exe and hands off to the swap helper. Returns true once the
    /// caller should exit so the helper can complete the swap; false if the download
        ...
        CancellationToken cancellationToken)
    {
        ...
        string appName = Path.GetFileNameWithoutExtension(currentExePath);
        string updateDir = Path.Combine(Path.GetTempPath(), $"{appName}_update");
        ...
        LaunchSwapHelper(updateDir, currentExePath!, newExePath, currentProcess.Id);
        return true;
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            using Stream sourceStream = await response.Content
                .ReadAsStreamAsync()
                .ConfigureAwait(false);
    ...
    /// <summary>
    /// Spawns a detached PowerShell script that waits for this process to exit, moves
    /// the downloaded exe over the current one, relaunches it, then deletes itself.
    /// </summary>
